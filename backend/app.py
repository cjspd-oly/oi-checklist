...
def get_note():
    ...
    ''', (request.user_id, name, source, year)).fetchone()
...
def google_start():
    # Begin Google login (new or existing account)
    google = OAuth2Session(GOOGLE_CLIENT_ID, redirect_uri=GOOGLE_CALLBACK_URL, scope=GOOGLE_SCOPE)
    # Ask for consent so we reliably get refresh tokens if you ever need them later
    ...
    _ensure_state_map()
    # For "start" we don't have a user; just mark as login flow
    app.oauth_state_map[state] = {"flow": "login", "redirect_to": "/"}
    return redirect(auth_url)
...
def google_link():
    ...
    db = get_db()
    session_row = db.execute("SELECT user_id FROM sessions WHERE session_id = ?", (session_token,)).fetchone()
    if not session_row:
        return jsonify({"error": "Invalid or expired session"}), 401
    ...
    google = OAuth2Session(GOOGLE_CLIENT_ID, redirect_uri=GOOGLE_CALLBACK_URL, scope=GOOGLE_SCOPE)
    auth_url, new_state = google.authorization_url(
        GOOGLE_AUTH_URL,
        state=state,             # preserve caller-provided state
        access_type="offline",
        prompt="consent"
    )
...
def google_callback():
    ...
    if identity:
        ...
        user_id = db.execute("SELECT id FROM users WHERE username = ?", (base_username,)).fetchone()["id"]
        db.execute(
            "INSERT INTO auth_identities (user_id, provider, provider_user_id, display_name) VALUES (?, ?, ?, ?)",
            (user_id, provider, google_sub, display_name)
        )
    ...
    # Create a new session for the user (like your other providers)
    session_id = str(uuid.uuid4())
    db.execute("INSERT INTO sessions (session_id, user_id) VALUES (?, ?)", (session_id, user_id))
    db.commit()

    # Redirect back to frontend (separate success pages keep parity with existing flows)
    redirect_to = "/"
...
def google_unlink():
    ...
    db.execute("DELETE FROM auth_identities WHERE provider = ? AND user_id = ?", ("google", request.user_id))
    db.commit()
